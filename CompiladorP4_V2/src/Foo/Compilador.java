...
// Regla de parametros OK
  ...
  // Variables de los tokens a coger y apilar
  Token t;
  Simbolo s;
  Tipo_variable tpV;
  Clase_parametro clPar;
    try {
      clase_parametros();
      tipos_variables();
      identificadores();
        try {
                        ...
                        s = tabla.buscar_simbolo();

                        // Obtencion del tipo de variable y la clase de parametro
                        tpV = s.getTipo();

                        clPar = s.getParametro();

                        // El simbolo del parametro no existe y se mete en la tabla
                tabla.introducir_parametro(t.image, Clasenivel, 0);
        }
        catch (Exception e) {
            // El simbolo ya existe en la tabla 
            SimboloRepetidoExcepcion(t.image);
        }
    } catch (ParseException e) {
     errorSintactico(e);
    }
